...
HELP_CHANNEL_ID = 1521076395462103091  # Канал для вопросов /помощь
...
# ============================================
# КОМАНДА: /помощь (С созданием тикета)
# ============================================

@bot.tree.command(name="помощь", description="📩 Задать вопрос о сервере")
@app_commands.guilds(discord.Object(id=GUILD_ID))
async def help_command(interaction: discord.Interaction):
    """Позволяет участникам задать вопрос о сервере"""
    await interaction.response.send_modal(HelpModal())

class HelpModal(discord.ui.Modal, title="📩 Задать вопрос о сервере"):
    question = discord.ui.TextInput(
        label="✏️ Ваш вопрос",
        placeholder="Напишите ваш вопрос о сервере...",
        required=True,
        style=discord.TextStyle.paragraph,
        max_length=1000
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.set_footer(text="✦ KURAI • Вопрос от участника")
        
        if interaction.guild.icon:
            embed.set_thumbnail(url=interaction.guild.icon.url)

        view = HelpButtonsView(user=interaction.user, question=self.question.value)

        await target_channel.send(
            content=f"📩 **Новый вопрос от {interaction.user.mention}**",
            embed=embed,
            view=view
        )

        await interaction.response.send_message(
            "✅ Ваш вопрос отправлен! Ответ появится в ближайшее время. 📩",
            ephemeral=True
        )
        ...
        try:
            ...
            dm_embed.set_footer(text="✦ KURAI • Спасибо за ваш вопрос!")
...
class HelpButtonsView(discord.ui.View):
    """Кнопки для обработки вопроса с созданием тикета"""
    ...
    @discord.ui.button(label="✅ Принять и создать канал", style=discord.ButtonStyle.success, custom_id="accept", emoji="✅")
    async def accept_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.is_processed:
            await interaction.response.send_message("❌ Этот вопрос уже обработан!", ephemeral=True)
            return

        if not is_staff(interaction.user):
            await interaction.response.send_message("❌ У вас нет прав на обработку вопросов!", ephemeral=True)
            return
        ...
        # Создаём канал
        channel_name = f"тикет-{self.user.name}-{self.user.discriminator}".lower()
        overwrites = {
            guild.default_role: discord.PermissionOverwrite(view_channel=False),
            self.user: discord.PermissionOverwrite(view_channel=True, send_messages=True, read_messages=True),
            interaction.user: discord.PermissionOverwrite(view_channel=True, send_messages=True, read_messages=True),
            guild.me: discord.PermissionOverwrite(view_channel=True, send_messages=True, read_messages=True)
        }
        
        # Добавляем доступ для всех Staff
        ...
        try:
            channel = await guild.create_text_channel(
                ...
                topic=f"Вопрос от {self.user.name} (ID: {self.user.id})"
            )
        except Exception as e:
        ...
        embed = discord.Embed(
            title="💬 Новый тикет",
            description=(
                f"**Вопрос от {self.user.mention}:**\n{self.question}\n\n"
                f"**Саппорт:** {interaction.user.mention}\n"
                "Общайтесь здесь для решения вопроса."
            ),
            color=COLORS['success']
        )
        embed.set_footer(text="✦ KURAI • Тикет создан")
        await channel.send(embed=embed)

        # Кнопка для закрытия тикета
        close_view = CloseTicketView(channel=channel, user=self.user, staff=interaction.user)
        await channel.send("🔒 **Для закрытия тикета нажмите кнопку ниже:**", view=close_view)

        # Ответ в основном канале
        embed_response = discord.Embed(
            title="✅ Вопрос принят и создан канал",
            description=f"Вопрос от {self.user.mention} принят!",
            color=COLORS['success']
        )
        embed_response.add_field(name="👤 Обрабатывает", value=interaction.user.mention, inline=True)
        embed_response.add_field(name="💬 Канал", value=channel.mention, inline=True)
        embed_response.add_field(name="🕐 Время", value=discord.utils.format_dt(datetime.now(), 'F'), inline=True)
        embed_response.set_footer(text="✦ KURAI • Вопрос в обработке")
        ...
        try:
            dm_embed = discord.Embed(
                title="✅ Ваш вопрос принят!",
                description=(
                    f"**Ваш вопрос:**\n{self.question}\n\n"
                    f"Его обрабатывает **{interaction.user.display_name}**.\n"
                    f"Канал для общения: {channel.mention}\n\n"
                    "Ожидайте ответа в ближайшее время! 📩"
                ),
                color=COLORS['success']
            )
            dm_embed.set_footer(text="✦ KURAI • Спасибо за ожидание!")
        ...
        if log_channel:
            log_embed = discord.Embed(
                title="📝 Создан тикет",
                description=f"Вопрос от {self.user.mention} принял {interaction.user.mention}",
                color=COLORS['success'],
    ...
    @discord.ui.button(label="❌ Отказ", style=discord.ButtonStyle.danger, custom_id="reject", emoji="❌")
    async def reject_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_modal(RejectReasonModal(user=self.user, question=self.question, original_interaction=interaction))


class CloseTicketView(discord.ui.View):
    """Кнопка для закрытия тикета"""
    def __init__(self, channel, user, staff):
    ...
    async def close_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Проверяем, что нажавший имеет доступ к каналу
        if not is_staff(interaction.user) and interaction.user.id != self.user.id:
        ...
        embed = discord.Embed(
            title="🔒 Тикет закрыт",
            description=(
                f"Тикет созданный для {self.user.mention} закрыт.\n"
                f"Закрыл: {interaction.user.mention}"
            ),
            color=COLORS['danger']
        )
        embed.set_footer(text="✦ KURAI • Тикет закрыт")
        await self.channel.send(embed=embed)

        # Убираем доступ у всех, кроме саппорта и участника (для архива)
        for role_id in STAFF_ROLES:
            role = interaction.guild.get_role(role_id)
            if role:
                await self.channel.set_permissions(role, view_channel=False)
        ...
        if log_channel:
            log_embed = discord.Embed(
                title="📝 Тикет закрыт",
                description=f"Тикет {self.channel.mention} закрыт {interaction.user.mention}",
                color=COLORS['danger'],
                timestamp=datetime.now()
            )
            await log_channel.send(embed=log_embed)
        ...
        # Уведомление автору
        try:
            await self.user.send(f"🔒 Ваш тикет **{self.channel.name}** закрыт.\nЗакрыл: {interaction.user.display_name}")
        except:
            pass
...
class RejectReasonModal(discord.ui.Modal, title="❌ Причина отказа"):
    reason = discord.ui.TextInput(
        ...
        placeholder="Укажите причину, по которой вопрос отклонён",
        required=True,
        style=discord.TextStyle.paragraph,
        max_length=500
    )

    def __init__(self, user, question, original_interaction):
        super().__init__()
        self.user = user
        self.question = question
    ...
    async def on_submit(self, interaction: discord.Interaction):
        embed = discord.Embed(
            title="❌ Вопрос отклонён",
            description=f"Вопрос от {self.user.mention} отклонён.",
            color=COLORS['danger']
        )
        embed.add_field(name="👤 Отклонил", value=interaction.user.mention, inline=True)
        embed.add_field(name="📝 Причина", value=self.reason.value, inline=False)
        embed.add_field(name="🕐 Время", value=discord.utils.format_dt(datetime.now(), 'F'), inline=True)
        embed.set_footer(text="✦ KURAI • Вопрос отклонён")
        
        await interaction.response.send_message(embed=embed)

        try:
            dm_embed = discord.Embed(
                title="❌ Ваш вопрос отклонён",
                description=(
                    f"**Ваш вопрос:**\n{self.question}\n\n"
                    f"**Причина отказа:**\n{self.reason.value}\n\n"
                    "Если у вас есть дополнительные вопросы, вы можете задать их снова. 📩"
                ),
                color=COLORS['danger']
            )
            dm_embed.set_footer(text="✦ KURAI • Свяжитесь с администрацией при необходимости")
            await self.user.send(embed=dm_embed)
        except:
            pass
        ...
        if log_channel:
            log_embed = discord.Embed(
                title="📝 Вопрос отклонён",
                description=f"Вопрос от {self.user.mention} отклонил {interaction.user.mention}",
                color=COLORS['danger'],
                timestamp=datetime.now()
            )
